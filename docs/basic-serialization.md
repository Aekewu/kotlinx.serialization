...
encode data using the [Json.encodeToString] function. It serializes the object
...
with a property in a primary constructor and move validation to the `init { ... }` block.
...
> 'ignoreUnknownKeys' feature is explained in the [Ignoring Unknown Keys section](json-configuration.md#ignoring-unknown-keys) section.
...
> See [Encoding defaults](json-configuration.md#encoding-defaults) section on how this behavior can be configured for JSON. 
...
> The corresponding feature is explained in the [Coercing input values](json-configuration.md#coercing-input-values) section. 
...
Generic classes in Kotlin provide type-polymorphic behavior and are also enforced by Kotlin serialization at 
